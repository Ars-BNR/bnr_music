...
const CardSongs = ({ track, active = false }: TrackItemProps) => {
  ...
  return (
    <div
      onClick={play}
      className={`
      cursor-pointer flex flex-col grow justify-around max-w-[172px]  p-[12px] bg-[#5801E1] rounded-[12px] gap-[10px] hover:bg-[#7A1FE3] hover:brightness-110 transition-all duration-300 relative 
      ${active ? "bg-red-900" : ""}
    `}
    >
      ...
      <div className="flex flex-col justify-center items-center gap-[2px] text-center">
        ...
        <p className="text-[#B6A295] text-[12px] mb-[4px]">{track.author}</p>
      </div>
    </div>
  );
};
